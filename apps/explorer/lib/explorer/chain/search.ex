defmodule Explorer.Chain.Search do
  ...
  defp maybe_run_ens_task(%PagingOptions{key: nil}, query_string, options) do
    Task.async(fn -> search_ens_name(query_string, options) end) |> dbg()
  end
  ...
  defp search_ens_name(search_query, options) do
    ...
    with true <- Regex.match?(~r/\w+\.\w+/, trimmed_query) |> dbg(),
         result when is_map(result) <- ens_domain_name_lookup(search_query) |> dbg() do
      [
        result[:address_hash]
